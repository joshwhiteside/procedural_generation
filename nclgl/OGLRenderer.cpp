...
/*
void OGLRenderer::SetMultipleShaderLight(const Light &l) {
	glUniform3fv(glGetUniformLocation(currentShader->GetProgram(),
		"lightPos") , 4, (float*)&l.GetPosition());

	glUniform4fv(glGetUniformLocation(currentShader->GetProgram(),
		"lightColour"), 4, (float*)&l.GetColour());

	glUniform1fv(glGetUniformLocation(currentShader->GetProgram(),
		"lightRadius"), 4, &l.GetRadius());
...
//Sets the shader particle size.. if youre into that sort of thing
void	OGLRenderer::SetShaderParticleSize(float f) {
