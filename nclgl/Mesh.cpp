...
Mesh* Mesh::GenerateTriangle() {
	...
	glPatchParameteri(GL_PATCH_VERTICES, m->numVertices);
...
void Mesh::Draw() {
	...
	float incvalue = 0.05f;

	cycle+=incvalue;

	float calcvalue  = sin(cycle) * 3.142f;
	for(GLuint i =0; i<numVertices; ++i) {
	if(data != (GLfloat*) NULL) {
	data[(i*4)+0] = (GLfloat) (sin(calcvalue)*0.6f)+0.9f;
	data[(i*4)+1] = (GLfloat) (cos(calcvalue)*0.6f)+0.9f;
	data[(i*4)+2] = (GLfloat) (tan(calcvalue)*0.6f)+0.9f;
	data[(i*4)+3] = (GLfloat) (sin(calcvalue)*0.6f)+0.9f;
...
void Mesh::GenerateColours() {
	...
	if(indices) {
		for(GLuint i = 0; i < numIndices; i+=3) {
			...
			Vector4 colour = Vector4((float)(rand()%255) /255.0f,(float)(rand()%255) /255.0f,(float)(rand()%255) /255.0f,1.0f);
			//Vector4 colour(0.0f,1.0f,0.0f,1.0f);
			colours[a] = colour;
			colours[b] = colour;
			colours[c] = colour;
		}
		...
		for(GLuint i = 0; i < numVertices; i+=3) {
			...
			//Vector4 colour(0.0f,1.0f,0.0f,1.0f);
			Vector4 colour = Vector4((float)(rand()%255 / 255),(float)(rand()%255 / 255),(float)(rand()%255 / 255),1.0f);
			colours[i] = colour;

			//colour = Vector4((float)(rand()%255 / 255),(float)(rand()%255 / 255),(float)(rand()%255 / 255),1.0f);
			colours[i+1] = colour;
...
/* ADD TO START OF DRAW FOR MULTI COLOURS
glBindBuffer(GL_ARRAY_BUFFER, bufferObject[COLOUR_BUFFER]);
//glUnmapBuffer(GL_ARRAY_BUFFER);
...
float calcvalue  = sin(cycle) * 3.142;
for(int i =0; i<numVertices; ++i) {
if(data != (GLfloat*) NULL) {
data[(i*4)+0]   = (GLfloat) (sin(calcvalue)*0.6)+0.9;
data[(i*4)+1] = (GLfloat) (cos(calcvalue)*0.6)+0.9;
data[(i*4)+2] = (GLfloat) (tan(calcvalue)*0.6)+0.9;
data[(i*4)+3] = (GLfloat) (sin(calcvalue)*0.6)+0.9;
