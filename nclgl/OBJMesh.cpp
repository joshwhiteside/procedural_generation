...
*/
bool	OBJMesh::LoadOBJMeshMTL(std::string filename)	{
	...
	while(!f.eof()) {
		...
		if(currentLine == MTLNEW) {
			...
			if(!currentMTL.diffuse.empty()) {
				string s = string(TEXTUREDIR + currentMTL.diffuse);

				const char *ss = s.c_str();

				currentMTL.diffuseNum = SOIL_load_OGL_texture(s.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, 0);
