...
Renderer::Renderer(Window &parent) : OGLRenderer(parent)	{
	...
	debugShader = new Shader(SHADERDIR"TexturedVertex.glsl",
...
void Renderer::DrawStuff() {
	...
	if(Window::GetKeyboard()->KeyDown(KEYBOARD_1)) {no = 1; cout << "1";}
	if(Window::GetKeyboard()->KeyDown(KEYBOARD_2)) {no = 2; cout << "2";}
	if(Window::GetKeyboard()->KeyDown(KEYBOARD_3)) {no = 3; cout << "3";}
	if(Window::GetKeyboard()->KeyDown(KEYBOARD_4)) {no = 4; cout << "4";}
	if(Window::GetKeyboard()->KeyDown(KEYBOARD_5)) {no = 5; cout << "5";}


	switch(no){
	case(1):	quad->SetTexture(bufferColourTex); break;
	case(2):	quad->SetTexture(lightEmissiveTex); break;
	case(3):	quad->SetTexture(lightSpecularTex); break;
	case(4):	quad->SetTexture(bufferDepthTex); break;
	case(5):	quad->SetTexture(bufferNormalTex); break;
	//case(0):	quad->SetTexture(lightSpecularTex); break;
	}
