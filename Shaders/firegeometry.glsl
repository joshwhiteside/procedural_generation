...
//number of vertices in the bound VBO
	...
	//using square brackets, just like C++ arrays
		...
		float sizerate = (1.0 - (IN[i].colour.w)) * 6;
		...
		//gl_Position.x += particleSize * sizerate;
		gl_Position.y += particleSize * sizerate * 15;
		...
		gl_Position = gl_in[ i ].gl_Position;
		gl_Position.x += particleSize * sizerate;
		gl_Position.y += particleSize * sizerate;
		OUT.texCoord = vec2(1,0);
		...
		//bottom right
		gl_Position = gl_in[ i ].gl_Position;
		gl_Position.x += particleSize * (1.0-sizerate);
